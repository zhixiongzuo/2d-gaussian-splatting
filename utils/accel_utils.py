...
def fast_adam_update(param, grad, exp_avg, exp_avg_sq, lr, beta1=0.9, beta2=0.999, eps=1e-15, step=1):
...
def compute_laplacian_edge_map(rendered_image, median_normalize=True, percentile=95):
    ...
    laplacian_kernel = torch.tensor([
        ...
        [1, -4, 1],
        [0, 1, 0]
    ], dtype=torch.float32, device=rendered_image.device)
    laplacian = F.conv2d(
    ...
    if median_normalize:
        edge_strength = median_normalize_edge_map(edge_strength, percentile=percentile)
    edge_strength = F.avg_pool2d(
        edge_strength.unsqueeze(0).unsqueeze(0),
...
@torch.no_grad()
def median_normalize_edge_map(edge_map, percentile=95):
    median = torch.median(edge_map)
    max_val = torch.quantile(edge_map, percentile)
    normalized = (edge_map - median) / (max_val - median + 1e-8)
    normalized = torch.clamp(normalized, 0, 1)
    return normalized


def fast_median_normalize(edge_map, num_bins=256):
    hist = torch.histc(edge_map, bins=num_bins, device=edge_map.device)
    cumsum = torch.cumsum(hist, dim=0)
    total = edge_map.numel()
    median_bin = torch.searchsorted(cumsum, total / 2.0)
    median_val = median_bin.float() / num_bins * edge_map.max()
    return (edge_map - median_val) / (edge_map.max() - median_val + 1e-8)
